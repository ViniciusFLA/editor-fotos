import type { Translations } from './types';

const es: Translations = {
  common: {
    cancel: 'Cancelar',
    confirm: 'Confirmar',
    delete: 'Eliminar',
    save: 'Guardar',
    close: 'Cerrar',
    ok: 'OK',
    yes: 'Sí',
    no: 'No',
  },
  editor: {
    toolbar: {
      undo: 'Deshacer',
      redo: 'Rehacer',
      group: 'Agrupar',
      ungroup: 'Desagrupar',
      preview: 'Vista previa',
      export: 'Exportar',
      rename: 'Doble clic para renombrar',
      format: 'Formato',
      scale: 'Escala',
    },
    sidebar: {
      uploads: 'Subidas',
      text: 'Texto',
      elements: 'Elementos',
      shapes: {
        rectangle: 'Rectángulo',
        circle: 'Círculo',
        line: 'Línea',
      },
      images: 'Imágenes',
      layers: 'Capas',
      ai: 'IA',
    },
    layers: {
      title: 'Capas',
      empty: 'Sin capas',
      bringToFront: 'Traer al frente',
      bringForward: 'Adelantar',
      sendBackward: 'Retroceder',
      sendToBack: 'Enviar al fondo',
      hide: 'Ocultar',
      show: 'Mostrar',
      lock: 'Bloquear',
      unlock: 'Desbloquear',
    },
    properties: {
      title: 'Propiedades',
      page: 'Página',
      position: 'Posición',
      size: 'Tamaño',
      transform: 'Transformación',
      text: {
        title: 'Texto',
        content: 'Contenido',
        fontSize: 'Tamaño',
        fontFamily: 'Fuente',
        fontWeight: 'Peso',
        normal: 'Normal',
        bold: 'Negrita',
        alignment: 'Alineación',
        left: 'Izquierda',
        center: 'Centro',
        right: 'Derecha',
        color: 'Color',
        systemFonts: 'Fuentes del Sistema',
        googleFonts: 'Fuentes Google',
      },
      shape: {
        title: 'Forma',
        fill: 'Relleno',
        stroke: 'Borde',
        strokeWidth: 'Ancho',
      },
      crop: {
        title: 'Recortar',
        enterMode: 'Recortar',
        apply: 'Aplicar',
        cancel: 'Cancelar',
      },
      filters: {
        title: 'Filtros',
        brightness: 'Brillo',
        contrast: 'Contraste',
        saturation: 'Saturación',
        blur: 'Desenfoque',
        grayscale: 'B/N',
        on: 'Activado',
        off: 'Desactivado',
      },
      background: {
        title: 'Fondo',
        type: 'Tipo',
        none: 'Ninguno',
        solidColor: 'Color Sólido',
        image: 'Imagen',
        linearGradient: 'Degradado Lineal',
        radialGradient: 'Degradado Radial',
        start: 'Inicio',
        end: 'Fin',
        direction: 'Dirección',
        url: 'URL',
      },
    },
    pages: {
      add: 'Añadir Página',
      delete: 'Eliminar Página',
      deleteConfirm: '¿Eliminar esta página?',
      newPage: 'Nueva Página',
      custom: 'Personalizado',
      presets: {
        instagramSquare: 'Cuadrado de Instagram',
        instagramPortrait: 'Retrato de Instagram',
        storiesReels: 'Stories / Reels',
        facebookLandscape: 'Horizontal de Facebook',
        youtubeThumbnail: 'Miniatura de YouTube',
      },
    },
    canvas: {
      loading: 'Cargando canvas...',
      empty: 'Canvas vacío',
      emptyHint: 'Añade imágenes, textos o formas para comenzar',
    },
    contextMenu: {
      copy: 'Copiar',
      paste: 'Pegar',
      duplicate: 'Duplicar',
      delete: 'Eliminar',
      group: 'Agrupar',
      ungroup: 'Desagrupar',
    },
    save: {
      saving: 'Guardando...',
      saved: 'Guardado',
      unsaved: 'No guardado',
      error: 'Error al guardar',
    },
    zoom: {
      in: 'Acercar',
      out: 'Alejar',
      reset: 'Restablecer Zoom (100%)',
      fit: 'Ajustar a Pantalla',
    },
    export: {
      format: 'Formato',
      scale: 'Escala',
      png: 'PNG',
      jpg: 'JPG',
      webp: 'WEBP',
    },
    language: {
      label: 'Idioma',
      ptBR: 'Português (Brasil)',
      en: 'English',
      es: 'Español',
    },
    ai: {
      detectText: 'Detectar texto',
      detecting: 'Detectando texto...',
      detected: '{count} textos detectados',
      selectImageHint: 'Selecciona una imagen para detectar texto.',
      convertAll: 'Convertir todos',
      editText: 'Editar texto',
      ignore: 'Ignorar',
      confidence: 'Confianza: {value}%',
      statusConverted: 'Editable',
      statusArmed: 'Editando',
      error: {
        requiresSingleImage: 'Selecciona una imagen.',
        imageFetchFailed: 'No se pudo cargar la imagen.',
        httpError: 'Error al detectar texto. Inténtalo de nuevo.',
        serviceUnavailable: 'Servicio no disponible temporalmente. Inténtalo en unos instantes.',
        pageRemoved: 'La página fue eliminada.',
        imageRemoved: 'La imagen fue eliminada.',
        alreadyProcessed: 'Esta imagen ya tiene texto detectado.',
        noTextDetected: 'No se encontró ningún texto.',
        allDetectionsFiltered: 'Se detectaron textos, pero ninguno alcanzó la confianza mínima.',
        inpaintingFailed: 'No se pudo reconstruir el fondo de esta imagen.',
        staleResult: 'La imagen cambió antes de completar la detección.',
        unknown: 'Error desconocido.',
      },
    },
  },
  textDefault: 'Tu texto',
  pageDefault: 'Página',
  imageDefault: 'Imagen',
  textLayerDefault: 'Texto',
  groupDefault: 'Grupo',
  uploadError: 'Error al cargar imagen.',
};

export default es;
